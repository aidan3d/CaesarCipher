...
public class CaesarCipher {
    ...
    /***
     * Checks whether a character is sitting in the seat (i.e., whether a specific chacter is
     * "sitting on the 'bus'!"
     * @param currentPersonOfInterest The current character being looked at
     * @param alphabets Basically, our solution set of available characters to encode. We can encode those on this list
     * @return A signal flag, rasied if a chracter is to be found in the provided lower- and upper-case "alphabets" string
     */
    ...
    /**
     * The "engine" of the program—this method does the actual leg-work (as all engines do!)!
     * @param bus The message we wish to encode. Thought of as a "bus" carrying "passengers" (letters of the alphabet)
     * @param seatsMoved The number of spaces we want to move the letters of the alphabet, i.e., "3" moves "a" to "d," etc.
     *                   The analogy here is that we "move" passengers in a single row of "seats" up by a certain number of
     *                   those seats—we "slide" them up, if you will
    ...
    public String encrypt( String passengers, int seatsMoved ) {
        ...
        String IDENTITIES = "abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ";  // "People" we're looking for
        ...
        for ( int i = 0; i < bus.length(); i++ ) {
            ...
            if ( isHuman( seat, IDENTITIES ) ) {
                // bus.charAt(i) = alter(seat) = seat;
